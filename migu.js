...
(function () {
    ...
    let lastBackquote = 0, backquoteNum = 1, iframe2, forceEnableIntervalID, downloading = 0, style2;
                    ...
                    if (event.data[0] == "audioVideoCapturer") {
                        function addToList() {
                            if (iframe2.contentDocument) {
                                ...
                                if (event.data[2]) {
                                    elt.outerHTML = "<a id=\"" + id + "\" style=\"text-decoration: underline; cursor: pointer;\">" + event.data[1].slice(0, 20) + "</a>\r\n";
                                    iframe2.contentDocument.getElementById(id).onclick = function () {
                                        window.postMessage(["audioVideoCapturerDownload", event.data[2], event.data[1]], "*");
                                        for (let i = 0; i < frames.length; i++) {
                                            frames[i].postMessage(["audioVideoCapturerDownload", event.data[2], event.data[1]], "*");
                                        }
                                        downloading++;
